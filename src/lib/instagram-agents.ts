...
export interface InstagramPipelineResult {
  ...
  // Суммарная стоимость 5 LLM-вызовов pipeline (для AI cost-cap).
  costUsd: number;
}
...
async function runAnalyzer(
  imageUrls: string[],
  userContext?: string,
): Promise<{ data: AnalyzerResult; costUsd: number }> {
  ...
  return {
    data: extractJson(text) as AnalyzerResult,
...
async function runStrategist(
  analysis: AnalyzerResult,
): Promise<{ data: StrategyResult; costUsd: number }> {
  const result = await getOpenRouter().chat.completions.create({
    model: VISION_MODEL,
    messages: [
  ...
  const text = result.choices[0]?.message?.content?.trim() || "";
  return {
    data: extractJson(text) as StrategyResult,
    costUsd: computeCostFromUsage(result.usage, VISION_MODEL),
  };
}
...
async function runCopywriter(
...
): Promise<{ data: CopywriterResult; costUsd: number }> {
  ...
  const text = result.choices[0]?.message?.content?.trim() || "";
  return {
    data: extractJson(text) as CopywriterResult,
    costUsd: computeCostFromUsage(result.usage, VISION_MODEL),
  };
}
...
async function runVisualEditor(
  analysis: AnalyzerResult,
  strategy: StrategyResult
): Promise<{ data: VisualEditorResult; costUsd: number }> {
  const result = await getOpenRouter().chat.completions.create({
    model: VISION_MODEL,
    messages: [
  ...
  const text = result.choices[0]?.message?.content?.trim() || "";
  return {
    data: extractJson(text) as VisualEditorResult,
    costUsd: computeCostFromUsage(result.usage, VISION_MODEL),
  };
}
...
async function runScheduler(
  recentPosts: string[],
): Promise<{ data: SchedulerResult; costUsd: number }> {
  const now = new Date();
  ...
  const text = result.choices[0]?.message?.content?.trim() || "";
  return {
    data: extractJson(text) as SchedulerResult,
    costUsd: computeCostFromUsage(result.usage, VISION_MODEL),
  };
}
...
export async function runInstagramPipeline(
  ...
  const [analyzerOut, schedulerOut] = await Promise.all([
    runAnalyzer(imageBase64Urls, userContext),
    runScheduler(recentPostDates),
  ]);
  const analysis = analyzerOut.data;
  const schedule = schedulerOut.data;
  totalCostUsd += analyzerOut.costUsd + schedulerOut.costUsd;
  ...
  const strategistOut = await runStrategist(analysis);
  const strategy = strategistOut.data;
  totalCostUsd += strategistOut.costUsd;
  ...
  const copywriterOut = await runCopywriter(analysis, strategy, userContext, smmProfile, contentPlanBrief);
  const copy = copywriterOut.data;
  totalCostUsd += copywriterOut.costUsd;
  console.log(`[Instagram Pipeline] Copy: ${copy.caption.substring(0, 50)}...`);

  // Phase 4: Visual Editor (needs analysis + strategy)
  const visualOut = await runVisualEditor(analysis, strategy);
  const visual = visualOut.data;
  totalCostUsd += visualOut.costUsd;
  console.log(`[Instagram Pipeline] Visual: score=${visual.visual_score}, order=${visual.final_order}`);

  return { analysis, strategy, copy, visual, schedule, costUsd: totalCostUsd };
}
