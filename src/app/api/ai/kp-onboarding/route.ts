...
export async function POST(req: NextRequest) {
  try {
    ...
    // редактора до кнопки «Сохранить настройки КП». Менеджер проходил семь шагов,
    // видел красивый результат, уходил — и клиенту уезжало дефолтное КП, хотя в
    // чек-листе стояла зелёная галочка «дизайн настроен».
    await prisma.master.update({
      where: { id: master.id },
      data: {
        kpConfig: result.config as unknown as object,
