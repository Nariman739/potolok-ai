...
export async function POST(
  ...
  try {
    ...
    // Раз ушло в цех — объект точно в работе. Ручной этап ниже «в цеху»
    // (например «согласовали») теперь только мешает: сбрасываем, чтобы
    // авто-расчёт показал «В цеху». Ручные «смонтировали/закрыл» не трогаем —
    // мастер мог дослать одну комнату уже после монтажа.
    await prisma.measurementObject.updateMany({
      where: { id: obj.id, manualStage: { in: ["measured", "calculated", "sent", "viewed", "confirmed"] } },
